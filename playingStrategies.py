...
def h_alphabeta_search(game, state, cutoff=cutoff_depth(0)):
    ...
    @cache1
    def max_value(state, alpha, beta, depth, action_considerata):
        ...
        for a in game.actions(state):
            v2, _ = min_value(game.result(state, a), alpha, beta, depth + 1, a)
            if v2 > v:
                v, move = v2, a
                alpha = max(alpha, v)
            if v >= beta:
    ...
    @cache1
    def min_value(state, alpha, beta, depth,action_considerata):
        if game.is_terminal(state):
            return game.utility(state, player), None
        if cutoff(game, state, depth):
        ...
        v, move = +infinity, None
        for a in game.actions(state):
            v2, _ = max_value(game.result(state, a), alpha, beta, depth + 1, a)
            if v2 < v:
                v, move = v2, a
                beta = min(beta, v)
            if v <= alpha:
    ...
    return max_value(state, -infinity, +infinity, 0, None)
